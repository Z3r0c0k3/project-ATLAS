...
class WorkbookServiceTest(unittest.TestCase):
    def test_parse_reconcile_and_never_truncate_documents(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(output_workbook.active.cell(46, 3).value, "물품 40")
            self.assertGreaterEqual(len(Document(package_dir / "영수증_및_소명자료.docx").inline_shapes), 3)
            self.assertGreaterEqual(len(Document(package_dir / "계좌전체내역.docx").inline_shapes), 1)
