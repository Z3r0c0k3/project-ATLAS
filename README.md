...
- Aegis 장부와 우리은행 거래내역 Excel 자동 판별·파싱
- 이미지와 다중 페이지 PDF 증빙의 DOCX 삽입
- 계좌 캡처와 우리은행·IBK기업은행 전체 거래내역을 함께 수록하는 계좌전체내역 DOCX
...
우리은행 파일은 `거래일시, 적요, 기재내용, 찾으신금액, 맡기신금액, 거래후 잔액,
취급기관, 메모` 헤더로 자동 판별한다. 우리은행 인터넷뱅킹에서 내려받은 구형 `.xls`와
`.xlsx`를 지원하며, 파일명은 판별에 사용하지 않는다. 회비입금계좌는 IBK기업은행
거래내역 PDF를 사용한다.

증빙 파일명 앞에 `#15# 2026. 3. 1. 결제 설명.pdf`처럼 `#장부ID#`를 붙이면
동아리운영계좌(우리은행) 자료로 확정 매칭한다. 회비입금계좌(IBK기업은행)의 환불·입금검증 소명자료는
...
- `증빙자료/`: 업로드한 증빙 원본
- `원본자료/`: Aegis 장부, 우리은행 Excel, IBK기업은행 PDF 원본
- `검증_리포트.html`, `manifest.json`: 장부·증빙 검증 결과와 모든 파일의 SHA-256

실제 사용 절차와 계좌별 파일 준비 방법은 [사용자 매뉴얼](USER_MANUAL.md)을 참고한다.
...
- `backend/storage`, `.env`, 실제 은행 거래내역 `sample/*.xls`, `sample/*.xlsx`는 Git에서 제외된다.
